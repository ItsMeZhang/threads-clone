    ...
    <div id="AuthPage" class="w-full h-[100vh] h-screen pt-32">
        <div class="w-full">
            ...
            <div class="max-w-[350px] mx-auto px-2 text-white">
                ...
                <!-- Button Login dengan parameter github sbg parameter login -->
                <button 
                    @click="login('github')"
                    class="
                        flex 
...
useHead({
  title: 'Threads - Login Pages',
  meta: [
    { name: 'description', content: 'Threads App Clone - Login Page' }
  ],
})

watchEffect(() => {
    // Jika user berhasil login, arahkan keberanda
    if (user.value) {
        return navigateTo('/')
    }
})
